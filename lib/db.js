const data = {
    general: {
        NUMBER_SYNTAX_ERROR: "```Enter a valid contact number as per the syntax below:\n    1. XXXXXXXXXX\n    2. Tag the person\n    3. YYXXXXXXXXXX (YY- Country Code, without zeros)```",
        ...
        SUDO_PERMISSION: "```Hey, I am Sree Charan. I guess you are trying to use my commands in``` *{BotsApp.groupName}*```. The bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* .\n\n",
        ERROR_TEMPLATE: "```Looks like something went wrong. Need not worry. Here are some logs since when the bot was not responding as expected.```\n```---------```\n```🧐 Command:``` *{commandName}*\n```😎 From Me?:``` *{fromMe}*\n```🗣️ Was a reply?:``` *{isReply}*\n```👥 In a group?``` *{isGroup}*\n```📥 In Inbox?``` *{isPm}*\n```📸 Command with image?``` *{isImage}*\n```🕺🏻 Is Bot group admin?``` *{isBotGroupAdmin}*\n```📈 Was Sender group admin?``` *{isSenderGroupAdmin}*\n```🫂 Was sender sudo?``` *{isSenderSudo}*\n```⚠️ Error:``` \n*{err}*\n```---------```\n_To figure out what exactly went wrong, please report/raise the issue on our support chat at_ https://chat.whatsapp.com/Gc8gNMoue2uHyd6xhFwvEP",
        SUCCESSFUL_CONNECTION: "*Sree Charan App successfuly integrated.*\n```It is currently working in``` *{worktype}* ```mode.```\n"
    },
    add: {
        DESCRIPTION: "Module to add a person to a group.",
        EXTENDED_DESCRIPTION: "```Add new people to a group by entering their mobile number as per the format mentioned below.\n    1. XXXXXXXXXX\n    2. YYXXXXXXXXXX ()\n\nFor example -\n``` *.add 9861212121*",
        NUMBER_SYNTAX_ERROR: "```Valid formats -\n    1. XXXXXXXXXX\n    2. YYXXXXXXXXXX\n\nFor example-```\n*.add 9861212121*\n*.add 919861212121*",
        NO_ARG_ERROR: "```Enter the number you want to add.\n\nFor instance,```  *.add <NUMBER>* .",
        NO_24HR_BAN: "```The number entered cannot be added back before 24 hours.```",
        ALREADY_MEMBER: "```The number entered is already a member of this group.```",
        NOT_ON_WHATSAPP: "```The number you're trying to add isn't available on WhatsApp.\nPlease verify the number again.```",
    ...
    block: {
        ...
        NOT_BLOCK_BOT: "```I can't block myself```"
    },
    carbon: {
    ...
    demote: {
        ...
        EXTENDED_DESCRIPTION: "```Use this module to demote a person from admin by entering the person's mobile number. Valid Syntaxes -\n    1. XXXXXXXXXX\n    2. YYXXXXXXXXXX ()\n\nFor example``` -\n*.demote 9861212121*",
        ...
        MESSAGE_NOT_TAGGED: "```Reply/tag/enter contact number of the person to be demoted.```"
    },
    disappear: {
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION: "```Get the profile picture of the group in a group conversation or the profile picture in personal chat.```",
        IMAGE_CAPTION: "```Here is the dp.```",
    ...
    help: {
        ...
        HEAD: "*Sree Charan Menu* \nUse *.help module* command for detailed info on a module.",
        TEMPLATE: "\n\n*Command* - ```{}```\n*Info* - ```{}```",
        COMMAND_INTERFACE: " *Commands I can run on* \n\n",
        COMMAND_INTERFACE_TEMPLATE: " *Triggers -* ```{}```\n *Info -* {}",
        FOOTER: "```\n\nClick button below to test run the module.```"
    },
    invite: {
    ...
    neko: {
        ...
        EXTENDED_DESCRIPTION: "```Use this module to paste your text to a pastebin (NEKOBIN). Enter text with the command```  *.neko* .",
    ...
    promote: {
        ...
        EXTENDED_DESCRIPTION: "```Use this module to promote a member to admin. You can enter the person's mobile number as per the format below. Valid Syntaxes -\n    1. XXXXXXXXXX\n    2. YYXXXXXXXXXX ()\n\nFor example-\n``` *.promote 9861212121*",
        NOT_A_GROUP: "```This command is only applicable in a group chat.```",
        BOT_NOT_ADMIN: "```Sorry, dont have the permission to do so since I am not an admin.```",
        PERSON_NOT_IN_GROUP: "```Person is not in the group.```",
        MESSAGE_NOT_TAGGED: "```Reply/tag/enter contact number of the person to be promoted.```"
    },
    remove: {
        DESCRIPTION: "Module to remove a person from a group.",
        EXTENDED_DESCRIPTION: "```Use this module to remove people from a group by tagging them```  *.remove @<person-to-remove>*  ```or replying to them```  *.remove*.",
    ...
    setdp: {
        ...
        EXTENDED_DESCRIPTION: "```Use this module to change the group's icon. Tag image with the command or send the desired image with caption as the command```",
    ...
    sticker: {
        ...
        EXTENDED_DESCRIPTION: "```Use this module to convert any image from your chat to a sticker. Reply to an image message with the command```  *.sticker*  ```to convert and send that image as a sticker.```",
        TAG_A_VALID_MEDIA_MESSAGE: "```Please tag a valid image/video/gif message to convert to sticker.```",
        DOWNLOADING: "```Your sticker is downloading. Please wait...```"
    },
    stoi: {
        DESCRIPTION: "Module to convert sticker to image",
        EXTENDED_DESCRIPTION: "```Use this module to convert any sticker from your chat to an image. Reply to a sticker message with the command```  *.stoi*  ```to convert and send that sticker as an image.```",
        ...
        TAG_A_VALID_STICKER_MESSAGE: "```Please tag a valid sticker message to convert to an image.```",
    ...
    unblock: {
        ...
        NUMBER_SYNTAX_ERROR: "```Enter a valid contact number. Valid syntax,\n    1. XXXXXXXXXX\n    2. Tag the person\n    3. +YYXXXXXXXXXX (YY- Country Code, without zeros)```",
        MESSAGE_NOT_TAGGED: "```Tag a message or enter a number.```",
    ...
    weather: {
        ...
        EXTENDED_DESCRIPTION: "```Obtain weather info by entering the city name.```",
        WEATHER_DATA: "*Temperature:* {tempInC} °C | {tempInF} °F\n*Min Temp:* {minTempInC} °C | {minTempInF} °F\n*Max Temp:* {maxTempInC} °C | {maxTempInF} °F\n*Humidity:* {humidity}%\n*Wind:* {windSpeedInkmph} kmph | {windSpeedInmph} mph , {degree}°\n*Sunrise:* {sunriseTime}\n*Sunset:* {sunsetTime}\n\n\n*{weatherDescription}*\n{cityName} , {country}\n{dateAndTime}",
        CITY_NAME_REQUIRED: "```Please mention the city name to search weather data.```",
    ...
    yt: {
        ...
        EXTENDED_DESCRIPTION: "```Get the first 10 recommendations from YouTube with their authorname, timestamp and link. Mention the keywords that are required to be searched along with the command.```",
